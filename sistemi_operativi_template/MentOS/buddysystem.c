...
block_found:
	...
	/* We found a block with 2^k (8) page frames to satisfy a request
     * of 2^h (4) page frames. If h < k, then we can split the block with 2^k (8)
	 * pages until it is large 2^h pages (4), namely k == h.
    */ 

	// We can exploit size(=2^k) to have at each loop the address the page that
	// resides in the middle of the found block.
	unsigned int size = 1 << current_order;	// *2
